...
Those expressions should also incorporate a specific way to determine their dependence on a given set of explanatory variables like e.g. climate, landscape, competition, etc.
...
Package functions follow a notation convention that is very similar to the one followed in the equations of this Vignette.
The table below will identify variables and their

* $t$, $t_{1}$ and $t_{2}$: time points, where $t_{1}\leq t_{2}$.
* $\Delta$: time difference, $\Delta = t_{2} - t_{1}$
* $y_{1}$ and $y_{2}$: size at time points $t_{1}$ and $t_{2}$, respectively.
It is assumed throughout that $y_{1}\leq y_{2}$.
* $k$: growth rate parameter. The dependence of growth on a set of explanatory 
variables is conveyed through this parameter.
* $\mathcal{X}$: set of explanatory variables on which growth will depend on through $k$.
* $a_{i}$: coefficients that multiply variables in the set $\mathcal{X}$.
* $A$: asymptotic maximum size.
* $B$: offset term.
* $g\left(\Delta\right)$: growth function such that $y_{2}=y_{1}+g\left(\Delta\right)$.
...
We will derive time-independent (also known as age-independent) curves for several $g\left(\Delta\right)$ functions. These curves have all been derived from sigmoid age-dependent size curves, which implies that there is always an asymptotic maximum size $A$. Some curves are well-known and widely used (e.g. logistic, Schumacher, Gompertz, monomolecular) whereas others are more rarely employed (e.g. arctangent, hyperbolic tangent).

To convey the dependence of growth on a given set of explanatory variables $\mathcal{X}$ like e.g. climate, landscape, competition, we will use the aforementioned parameter $k$ as a linear function:

\begin{equation}
  k=\sum_{i=1}^{n} a_{i}\cdot \mathcal{X}_{i}
\end{equation}

which, in turn, multiplies age/time $t$. Many age-dependent size curves, and their corresponding age-independent expressions, are highly non-linear in $k$. Thus, a successful fitting strategy requires the implementation of a non-linear regression algorithm to calculate the coefficients $a_{i}$ that determine the effect of the corresponding explanatory variable. As we will see below, this strategy yields relatively simple expressions that can readily be implemented as time-independent growth functions.


In addition, term $B$ represents an (a priori unknown) offset in the growth curve. Nevertheless, as we will see below, this summative offset $B$ conveniently vanishes in all cases when $g\left(\Delta\right)$ or $k$ are expressed as a function of $\Delta$. Finally, in all expressions it is easy to check that $g\left(\Delta\right)=0$ when $\Delta=0$ or $k=0$ always, as expected.
...
The logistic curve, labelled *Autocatalytic* in Richards' original article, is one of the most widely used sigmoid functions to determine the growth of an individual plant or animal. In a general case, in which size depends on time $t$ as well as on a set of predictors (e.g. climatic, topographic), the size $y$ of an organism that obeys the logistic rule can be written:

\begin{equation}
  y=\frac{A}{1+e^{-\left(k\cdot t+B\right)}}
\end{equation}

As mentioned above, the dependence on predictors is done through $k$. To obtain a time-independent equation that determines growth between two time points $t_{1}$ and $t_{2}$, $t_{1}<t_{2}$, we start by writing:
