...
where $t_{1}<t_{2}$, then growth $\Delta_{y}$, defined as
$\Delta_{y}=y(t_{2})-y(t_{1})$, will simply be:
...
\end{equation}
```
Expressed this way, the new growth function $g\left(t_{1}, t_{2}\right)$
still depends explicitly on absolute age/time at $t_{1}$ and $t_{2}$,
...
$\Delta_{t}=t_{2}-t_{1}$, a relative quantity that is much easier to
obtain. That is, we want to be able to write:
