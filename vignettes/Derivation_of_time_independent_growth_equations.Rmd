...
where $t_{1}<t_{2}$, then growth , defined as
$y(t_{2})-y(t_{1})$, will simply be an unknown function $f\left(t_{1}, t_{2}\right)$.
Expressed this way, that function $f\left(t_{1}, t_{2}\right)$
depends explicitly on absolute age/time at $t_{1}$ and $t_{2}$,
which are usually unknown quantities in natural experiments. To
determine growth independent of age/time, we will seek an analytic
expression that is instead a function of the time interval
$\Delta=t_{2}-t_{1}$, a relative quantity that is much easier to
obtain. That is, we want to be find a new growth function $g$:
...
5.  Arc tangent
6.  Hyperbolic tangent
7.  Arctangent exponential
8.  Rational[8. Rational function](#rational)
...
#### 7. Arctangent exponential growth function
...
it is identical to a rescaled logistic.
...
points(t,Rtigre::td_size(dat, curve_type = "rational"), type = "l", lwd = 2, col = 6)
legend("bottomright", lty = 1, c("logistic", "schumacher", "gompertz", "monomolecular",
"arctangent", "rational"), lwd = 2, cex = 1.1, col = 1:6)
```
...
points(y1,Rtigre::ti_size(dat, "rational")-dat$y1,type="l",lwd=2,col=6)
