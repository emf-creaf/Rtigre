...
#'   dat <- Punci_IFN[j, ]
#'   r <- lapply(curves, function(x) fit_growth(dat, fo, log_transf = T, positive_rate = F, curve_type = x, verbose = F))
#'   dat <- Punci_IFN[-j, ]
#'   out[i, ] <- sapply(r, function(x) cor(predict(x, newdata = dat), (dat$y2-dat$y1))^2)
#' }
#' library(ggplot2)
#' library(gridExtra)
#' plot_list <- list()
